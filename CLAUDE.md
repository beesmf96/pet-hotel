...
roadmap and status: `notes/tasks.md`.
...
php notes/build.php   # renders stakeholder docs → docs/ (the GitHub Pages site)
```
...
3. A session log entry in `notes/log/` (rules: `notes/paper-trail.md`). Add an ADR, knowledge
...
markdown there, and keep its `.nojekyll`. Markdown lives under `notes/` and is coder-facing by
default. Only files with `audience: stakeholder` render to `docs/`. Never hand-edit a
generated `.html` — the hand-written exceptions are listed in `STATIC_DOCS` in `notes/build.php`.
